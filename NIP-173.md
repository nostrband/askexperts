...
The protocol assumes that receivers are actively listening (otherwise they might miss ephemeral events), the logic for signalling online status between sender and receiver is out of scope and is up to the applications.

The protocol does not provide a way for slow receivers to push back, the default assumption is that applications will choose relays that buffer ephemeral events long enough for slow receivers to catch up. 
...
except the first one includes `prev` tag with event id of previous chunk - this helps resolve the proper "branch"
if sender sends several streams using the same metadata.
...
* If `content` is empty, such event should be treated as keep-alive ping or status update.
