...
async function handleAskEvent(
  ...
  try {
    ...
    const bidPayload: UnsignedEvent = {
      ...
      content:
        "I'm a fake expert but I can provide a real answer to your question!",
      tags: [
        ["invoice", invoice],
        ["relay", BID_RELAY],
      ],
    ...
    try {
      ...
      if (successful > 0) {
        ...
        console.log(
          `Started question handler for bid payload ${signedBidPayload.id}`
        );
      } else {
...
async function handleQuestionForBid(
  ...
  return new Promise<void>((resolve, reject) => {
    ...
    timeoutId = setTimeout(() => {
      console.log(
        `Timeout reached for bid payload ${bidPayloadId}, closing subscription`
      );
      if (subscription) {
        subscription.close();
      }
    ...
    subscription = pool.subscribeMany(DEFAULT_RELAYS, [filter], {
      onevent: async (questionEvent: Event) => {
        try {
          console.log(
            `Received question event for bid payload ${bidPayloadId}: ${JSON.stringify(
              questionEvent
            )}`
          );
          ...
          const answerPayload = {
            content:
              "This is a fake answer to your question. In a real implementation, this would be a thoughtful response based on the expert's knowledge.",
            tags: [],
          };
      ...
      oneose: () => {
        console.log(
          `End of stored events for bid payload ${bidPayloadId}, now listening for new events`
        );
      },
    });
  });
...
if (
  process.argv[1].endsWith("fakeExpert.ts") ||
