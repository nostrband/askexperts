...
Client SHOULD include `e`-tag with either `bid payload` event id if this is the first question in the session, or the previous `answer`'s event id if this is a followup question.
